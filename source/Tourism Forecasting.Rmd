...
# date ranges
min(tsibble_data$month)
max(tsibble_data$month)

# length ranges
series.length = tsibble_data |> as_tibble() |> 
  group_by(series_name) |> summarise(length = n())
min(series.length$length)
max(series.length$length)
mean(series.length$length)
...
accu.with.best.model$.model[accu.with.best.model$.model=='ets'] = 'ETS'
...
# compare ARIMA ETS SNaive and NNAR
...
```{r T1 model}
# check ETS parameter on T1 series
